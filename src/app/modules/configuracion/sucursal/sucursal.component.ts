...
import { ICiudad } from '../ciudad/models/ciudad.model';
...
export class SucursalComponent implements OnInit,OnDestroy {
  ...
  listCiudad:ICiudad[]=[];
  paginationInfo: ResponsePagination<ISucursal> = {
    totalGlobal: 0,
    totalFiltered: 0,
    records: [],
  };
  constructor(private pageInfo: PageInfoService,
    ...
    this.pageInfo.setTitle('Sucursal');
  ...
  ngOnInit(): void {
    ...
    this.ciudadService.List().subscribe(res=>{
      this.listCiudad=res.data as ICiudad[];
    })
  }

  PaginateProvincia() {
    this.getEscalaSub = this.sucursalService
  ...
  save() {
    ...
    this.sucursalService.Save(this.form.value).subscribe({
      next: (response) => {
  ...
  edit(id: number) {
    ...
    this.sucursalService.getById(this.id_categoria).subscribe({
      next: (response) => {
  ...
  delete() {
    this.sucursalService.Delete(this.id_categoria).subscribe({
      next: (response) => {
        this.id_categoria = 0;
        this.PaginateProvincia();
