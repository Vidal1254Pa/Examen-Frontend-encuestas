...
export class PageInfoService {
  ...
  public description: BehaviorSubject<string> = new BehaviorSubject<string>('Dashboard');
  ...
  public calculateBreadcrumbs() {
    const asideBc = this.calculateBreadcrumbsInMenu('#kt_app_sidebar_menu');
  ...
  public calculateBreadcrumbsInMenu(
    ...
    const singleMenu = <HTMLAnchorElement>menu.querySelector('a.menu-item.without-sub.here');
    if(singleMenu) {
      result.push({
        title: singleMenu.querySelector('span.menu-title')?.innerHTML || '',
        path: singleMenu.getAttribute('routerlink') || '',
        isSeparator: true,
        isActive: false,
      });
      result.push({
